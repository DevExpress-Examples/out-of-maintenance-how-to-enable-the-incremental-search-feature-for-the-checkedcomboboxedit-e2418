...
Namespace WindowsApplication31
	...
	Partial Public Class nwindDataSet
		...
		Partial Public Class EmployeesDataTable
			...
			<Global.System.Diagnostics.DebuggerNonUserCodeAttribute()> _
			Public Function AddEmployeesRow(ByVal LastName As String, ByVal FirstName As String) As EmployeesRow
				Dim rowEmployeesRow As EmployeesRow = (CType(Me.NewRow(), EmployeesRow))
				Dim columnValuesArray() As Object = { Nothing, LastName, FirstName}
...
Namespace WindowsApplication31.nwindDataSetTableAdapters
	...
	Partial Public Class EmployeesTableAdapter
		...
		Private Sub InitAdapter()
			...
			Me._adapter.DeleteCommand.CommandText = "DELETE FROM `Employees` WHERE ((`EmployeeID` = ?) AND ((? = 1 AND `LastName` IS N" & "ULL) OR (`LastName` = ?)) AND ((? = 1 AND `FirstName` IS NULL) OR (`FirstName` =" & " ?)))"
			...
			Me._adapter.InsertCommand.CommandText = "INSERT INTO `Employees` (`LastName`, `FirstName`) VALUES (?, ?)"
			...
			Me._adapter.UpdateCommand.CommandText = "UPDATE `Employees` SET `LastName` = ?, `FirstName` = ? WHERE ((`EmployeeID` = ?) " & "AND ((? = 1 AND `LastName` IS NULL) OR (`LastName` = ?)) AND ((? = 1 AND `FirstN" & "ame` IS NULL) OR (`FirstName` = ?)))"
		...
		Private Sub InitCommandCollection()
			...
			Me._commandCollection(0).CommandText = "SELECT EmployeeID, LastName, FirstName FROM Employees"
		...
		Public Overridable Function Delete(ByVal Original_EmployeeID As Integer, ByVal Original_LastName As String, ByVal Original_FirstName As String) As Integer
		...
		Public Overridable Function Insert(ByVal LastName As String, ByVal FirstName As String) As Integer
		...
		Public Overridable Function Update(ByVal LastName As String, ByVal FirstName As String, ByVal Original_EmployeeID As Integer, ByVal Original_LastName As String, ByVal Original_FirstName As String) As Integer
			If (LastName Is Nothing) Then
				Throw New Global.System.ArgumentNullException("LastName")
			Else
			...
			End If
			Me.Adapter.UpdateCommand.Parameters(2).Value = (CInt(Fix(Original_EmployeeID)))
			If (Original_LastName Is Nothing) Then
				Throw New Global.System.ArgumentNullException("Original_LastName")
			Else
				Me.Adapter.UpdateCommand.Parameters(3).Value = (CObj(0))
				Me.Adapter.UpdateCommand.Parameters(4).Value = (CStr(Original_LastName))
			End If
			If (Original_FirstName Is Nothing) Then
				Throw New Global.System.ArgumentNullException("Original_FirstName")
			Else
